exports.handler = async function(event, context) {
  ...
  const priceMap = {
    ...
    // {CHECKOUT_SESSION_ID} is replaced by Stripe itself in the redirect URL;
    // the client sends it to verify-checkout-session.js to confirm the
    // subscription server-side instead of trusting the URL flag alone.
    const referralCode = body.referralCode ? body.referralCode.trim().toUpperCase() : '';
    const successUrl = referralCode
      ? `https://www.sugarscannerapp.com?subscribed=true&ref=${referralCode}&session_id={CHECKOUT_SESSION_ID}`
          : 'https://www.sugarscannerapp.com?subscribed=true&session_id={CHECKOUT_SESSION_ID}';
